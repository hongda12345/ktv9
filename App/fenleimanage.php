...
class fenleimanage{
    ...
    function index(){
        $title='分类管理';
        include 'App/views/fenleimanage.html';
    }
    function insert(){
        $data=$_POST;
    ...
    function show(){
        /*$mysql=new mysqli('localhost','root','','ktv',3306);
        $mysql->query('set names utf8');*/
        $data=$this->db->query("select * from sort")->fetch_all(MYSQLI_ASSOC);
    ...
    function delete(){
        ...
        /*$mysql=new mysqli('localhost','root','','ktv',3306);
        $mysql->query('set names utf8');*/
        $data=$this->db->query("delete from sort where sid=$ids");
        if($this->db->affected_rows){
            echo 'ok';
            exit;
        }
    ...
    function update(){
        ...
        /*$mysql=new mysqli('localhost','root','','ktv',3306);
        $mysql->query('set names utf8');*/
        $data=$this->db->query("update sort set $type='{$value}' where sid=$ids");
        if($this->db->affected_rows){
            echo 'ok';
            exit;
        }
